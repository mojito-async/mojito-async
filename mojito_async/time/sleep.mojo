...
# `sleep` is present and typed exactly as A1.4 will ship it and raises a
# precise error when invoked, rather than pretending to time out.  No hidden
# blocking, no allocation.
#
# A1.4 will replace this body with a real deadline-based park on the timer
# wheel; the signature is frozen here.


def sleep(ms: Int) raises:
    """Sleep the CURRENT task for `ms` milliseconds.

    NOT IMPLEMENTED in A1.1 — landed by the A1.4 timer lane.  Raises a
    descriptive error (never blocks or spins).  The signature is the stable
    A1.4 surface; keep callers compiling until the timer heap exists."""
