...
# Semaphore + Permit (spec §36), Condvar (A4.6, issue #60, spec §A5) and
...
# grant it back via FIFO handoff.  Condvar/Barrier additionally expose their
# own winner-cause constants (WINNER_READY/CANCELLED/TIMEOUT) so callers can
# interpret the `cause` cell threaded through `wait`.
