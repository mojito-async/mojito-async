...
#   T1  oversubscription guard: make_fiber()/bind() raise a catchable Error
#       (not a SIGILL trap on the vendored resume table's 64-row / 32-fiber
#       ceiling) before the 33rd live fiber is created.
...
comptime MAX_FIBERS = Fiber.MS_MAX_LIVE_FIBERS  # 32
...
def main() raises:
    ...
    # ------------------------------------------------------------------
    # T1 oversubscription guard.
    # ------------------------------------------------------------------
    var cnt0 = ms_live_stack_count()
    comptime count = Int(MAX_FIBERS) + 1
    ...
    for i in range(count):
        ...
        if i == Int(MAX_FIBERS):
            if not raised_here:
                failures.append("T1: 33rd live fiber was not refused")
            else:
                raised_33 = True
    if not raised_33:
        failures.append("T1: oversubscription did not raise on the 33rd fiber")
    if bound != Int(MAX_FIBERS):
        failures.append("T1: guard rejected a fiber below the ceiling")

    # destroy the 32 bound fibers (each frees its own stack); the 33rd stack
    # was never bound (the guard raised before ownership transfer), so free it
    # explicitly to restore the live count.
    for i in range(Int(MAX_FIBERS)):
        fibers[i].destroy()
    ms_stack_free(stacks[Int(MAX_FIBERS)])
