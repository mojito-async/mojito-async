...
struct CancelFlagHook(CancelHook):
    ...
    A3.5 (issue #64): conforms DIRECTLY to `mojito_async.scope.CancelHook`
    (not a re-listing of its base traits) so it satisfies scope.mojo's
    `record_failure[H: CancelHook]` generic bound — b2 checks a generic
    trait bound nominally against the trait name a struct actually
    declares, not structurally against an equivalent base-trait list (the
    #61 module header above predates #64's per-call generic hook seam and
    was written before this bound existed; matches the
    `RecordingCancel(CancelHook)` pattern in t30_failure_policy.mojo)."""
